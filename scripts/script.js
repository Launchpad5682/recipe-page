...
function creatingList(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    instruction.appendChild(inputDiv);
    instruction.appendChild(step);
    instruction.appendChild(brek);
...
for (let i = 0; i < recipeBook.length; i++) {
  let body = document.getElementsByClassName("ingredients")[0];
...
instruction.classList.add("instructions-heading");
...
for (let i = 0; i < instructions.length; i++) {
  ...
  instruction.appendChild(instruct);
  instructionDiv.append(instruction);
}

document.getElementsByTagName("body")[0].appendChild(instructionDiv);

let sourcePara = document.createElement("p");
sourcePara.innerHTML = source;
sourcePara.classList.add("source");

document.getElementsByTagName("body")[0].appendChild(sourcePara);
